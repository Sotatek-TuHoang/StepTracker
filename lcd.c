...
//	int k;
	if(Digit > 4){
		...
		else if(Value==0x09){LCD->WF8B[LCD_Frontplane_Pin[((2*Digit)-2)]] = (LCD_SEG_D | LCD_SEG_F | LCD_SEG_G); LCD->WF8B[LCD_Frontplane_Pin[((2*Digit)-1)]] = (LCD_SEG_A | LCD_SEG_B | LCD_SEG_C);}
...
void SegLCD_Clear(uint8_t Pos1, uint8_t Pos2, uint8_t Pos3, uint8_t Pos4){	//	be added
	if(Pos1 == 1){
...
void DisplayMinusSymbol(uint8_t position){	//	be added
	LCD->WF8B[LCD_Frontplane_Pin[((2*position)-2)]] = LCD_SEG_G; 
	LCD->WF8B[LCD_Frontplane_Pin[((2*position)-1)]] = LCD_CLEAR;
}	//End DisplayMinusSymbol
